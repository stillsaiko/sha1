...
func sha1{range_check_ptr, bitwise_ptr : BitwiseBuiltin*}(input : felt*, size : felt) -> (hash160 : felt):
	...
	compress_b_and_c_or_not_b_and_d:
		...
		let (_, lshift_5) = unsigned_div_rem(2 ** 5 * a, 2 ** 32)
		# B <<< 30
		...
		assert bitwise_ptr[4].x = 0x5A827999 + [w] + lshift_5 + rshift_27 + bitwise_ptr[3].x_or_y + e
	...
	compress_b_xor_c_xor_d_6ed9eba1:
		...
		assert bitwise_ptr[2].x = 0x6ED9EBA1 + [w] + lshift_5 + rshift_27 + bitwise_ptr[1].x_xor_y + e
	...
	compress_b_and_c_or_b_and_d_or_c_and_d:
		...
		assert bitwise_ptr[6].x = 0x8F1BBCDC + [w] + lshift_5 + rshift_27 + bitwise_ptr[4].x_or_y + e
	...
	compress_b_xor_c_xor_d_ca62c1d6:
		...
		assert bitwise_ptr[1].x = bitwise_ptr[0].x_xor_y
		assert bitwise_ptr[1].y = d
		assert bitwise_ptr[2].x = 0xCA62C1D6 + [w] + lshift_5 + rshift_27 + bitwise_ptr[1].x_xor_y + e
		assert bitwise_ptr[2].y = 0xFFFFFFFF
		tempvar b = a
		tempvar a = bitwise_ptr[2].x_and_y
		tempvar e = d
